		...
		function renderObject( object, scene, camera ) {
			...
			if ( object.isCSS2DObject ) {
				...
				if ( visible === true ) {
					...
					// Trace patch: whole-pixel positions keep label text sharp at devicePixelRatio 1.
					// Labels that cache their size (element._w/_h) are placed without percentage offsets.
					const px = _vector.x * _widthHalf + _widthHalf, py = - _vector.y * _heightHalf + _heightHalf;
					if ( element._w !== undefined ) element.style.transform = 'translate(' + Math.round( px - object.center.x * element._w ) + 'px,' + Math.round( py - object.center.y * element._h ) + 'px)';
					else element.style.transform = 'translate(' + ( - 100 * object.center.x ) + '%,' + ( - 100 * object.center.y ) + '%)' + 'translate(' + Math.round( px ) + 'px,' + Math.round( py ) + 'px)';
